...
export class MerchandiseService {
  ...
  async uploadPicture(barcode: string, file: Buffer) {
    const fileName = 'merchandise-' + barcode + '-' + uuid().slice(0, 8);
    const url = await this.ossService.uploadMerchandisePicture(file, fileName);
    return url;
  }
